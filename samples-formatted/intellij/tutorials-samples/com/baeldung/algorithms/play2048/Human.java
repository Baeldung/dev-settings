...
public class Human {
    ...
    public Board makeMove(Board input) {
        ...
        return Arrays.stream(Move.values()).parallel().map(input::move).filter(board -> !board.equals(input))
            .max(Comparator.comparingInt(board -> generateScore(board, 0))).orElse(input);
    }
    ...
    private int generateScore(Board board, int depth) {
        ...
        return board.emptyCells().stream().parallel()
            .flatMap(cell -> Stream.of(new Pair<>(cell, 2), new Pair<>(cell, 4))).mapToInt(move -> {
                ...
                LOG.debug("Calculated score for board {} and move {} at depth {}: {}", newBoard, move, depth,
                    calculatedScore);
    ...
    private int calculateScore(Board board, int depth) {
        return Arrays.stream(Move.values()).parallel().map(board::move).filter(moved -> !moved.equals(board))
            .mapToInt(newBoard -> generateScore(newBoard, depth)).max().orElse(0);
    }
