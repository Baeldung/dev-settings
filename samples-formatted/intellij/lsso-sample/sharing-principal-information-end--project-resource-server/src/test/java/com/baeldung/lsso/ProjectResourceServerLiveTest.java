...
public class ProjectResourceServerLiveTest {
    ...
    public void givenRequestWithPreAuthHeaders_whenRequestProjectsEndpoint_thenOk() throws Exception {
        RestAssured.given().header("BAEL-username", "customUsername").header("BAEL-authorities", "SCOPE_read")
            .get(PROJECT_RESOURCE_URL).then().statusCode(HttpStatus.OK.value()).and().body("size()", greaterThan(0));
    }
    ...
    public void givenRequestWithPreAuthHeaders_whenPostProject_thenCreated() throws Exception {
        ...
        RestAssured.given().header("BAEL-username", "customUsername").header("BAEL-authorities", "SCOPE_write")
            .contentType(ContentType.JSON).body(newProject).post(PROJECT_RESOURCE_URL).then()
    ...
    public void givenJustUsernameHeaders_whenRequestProjectsEndpoint_thenForbidden() throws Exception {
        RestAssured.given().header("BAEL-username", "customUsername").get(PROJECT_RESOURCE_URL).then()
            .statusCode(HttpStatus.FORBIDDEN.value());
    }

    @Test
    public void givenJustUsernameHeaders_whenRequestNonExistingEndpoint_thenNotFound() throws Exception {
        RestAssured.given().header("BAEL-username", "customUsername").get(PROJECT_RESOURCE_SERVER_BASE_URL + "/other")
