...
public class Oauth2ClientLiveTest {
    ...
    public void givenAuthorizationCodeGrant_whenLoginUsingOauth_thenSuccess() throws UnsupportedEncodingException {
        ...
        response = RestAssured.given().redirects().follow(false).cookie("AUTH_SESSION_ID", authSessionId)
            .formParams("username", USERNAME, "password", PASSWORD, "credentialId", "").post(kcPostAuthenticationUrl);
        assertThat(HttpStatus.FOUND.value()).isEqualTo(response.getStatusCode());
        ...
        // mimic oauth2login
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", clientSessionId)
        ...
        // check projects page
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", newClientSessionId)
        ...
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", newClientSessionId)
            .formParam("name", "newProjectName").formParam("_csrf", csrfToken).post(PROJECTS_RESOURCE_URL);
