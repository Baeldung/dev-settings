...
public class Human {
    ...
    public Board makeMove(Board input) {
        ...
        return Arrays.stream(Move.values()).parallel().map(input::move).filter(board -> !board.equals(input)).max(Comparator.comparingInt(
            board -> generateScore(board, 0))).orElse(input);
    }
