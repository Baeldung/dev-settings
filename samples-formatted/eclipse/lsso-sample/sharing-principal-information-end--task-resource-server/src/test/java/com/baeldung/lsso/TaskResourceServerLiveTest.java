...
public class TaskResourceServerLiveTest {
    ...
    public void givenRequestWithPreAuthHeaders_whenRequestProjectsEndpoint_thenOk() throws Exception {
        RestAssured.given().header("BAEL-username", "customUsername").header("BAEL-authorities", "SCOPE_read").get(PROJECT_RESOURCE_URL + "?projectId=1").then()
            .statusCode(HttpStatus.OK.value()).and().body("size()", greaterThan(0));
    }
    ...
    public void givenJustUsernameHeaders_whenRequestNonExistingEndpoint_thenNotFound() throws Exception {
        RestAssured.given().header("BAEL-username", "customUsername").get(TASK_RESOURCE_SERVER_BASE_URL + "/other").then().statusCode(HttpStatus.NOT_FOUND
            .value());
    }
