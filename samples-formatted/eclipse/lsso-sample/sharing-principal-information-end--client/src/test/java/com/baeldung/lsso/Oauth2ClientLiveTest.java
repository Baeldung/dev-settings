...
public class Oauth2ClientLiveTest {
    ...
    public void givenAuthorizationCodeGrant_whenLoginUsingOauth_thenSuccess() throws UnsupportedEncodingException {
        ...
        Response response = RestAssured.given().redirects().follow(false).get(CLIENT_AUTHORIZATION_URI);
        ...
        response = RestAssured.given().redirects().follow(false).get(fullAuthorizeUrl);
        ...
        response = RestAssured.given().redirects().follow(false).cookie("AUTH_SESSION_ID", authSessionId).formParams("username", USERNAME, "password", PASSWORD,
            "credentialId", "").post(kcPostAuthenticationUrl);
        assertThat(HttpStatus.FOUND.value()).isEqualTo(response.getStatusCode());
        ...
        // mimic oauth2login
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", clientSessionId).get(REDIRECT_URL + "?code=" + code + "&state=" + state);
        assertThat(HttpStatus.FOUND.value()).isEqualTo(response.getStatusCode());
        ...
        // check projects page
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", newClientSessionId).get(PROJECTS_RESOURCE_URL);
        ...
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", newClientSessionId).get(ADD_PROJECT_URL);
        assertThat(HttpStatus.OK.value()).isEqualTo(response.getStatusCode());
        ...
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", newClientSessionId).formParam("name", "newProjectName").formParam("_csrf",
            csrfToken).post(PROJECTS_RESOURCE_URL);
        ...
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", newClientSessionId).get(TASKS_RESOURCE_URL + "?projectId=1");
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK.value());
        assertThat(response.asString()).contains("Tasks : View all");

        // check tasks page with invalid query param
        response = RestAssured.given().redirects().follow(false).cookie("JSESSIONID", newClientSessionId).get(TASKS_RESOURCE_URL);
