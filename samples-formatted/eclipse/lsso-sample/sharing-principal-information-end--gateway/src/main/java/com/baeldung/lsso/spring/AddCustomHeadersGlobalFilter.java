...
public class AddCustomHeadersGlobalFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        return exchange.getPrincipal().filter(Authentication.class::isInstance).cast(Authentication.class).map(authentication -> addHeaders(exchange,
            authentication)).flatMap(chain::filter);
